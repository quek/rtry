...
### Windows パス権限
- `C:\Program Files\` は管理者権限でしか書き込めない。バイナリとデフォルトデータの配置先
- 設定ファイル (config.json) は `%ProgramData%\rtry\` に置く。`install.bat` が Users グループに変更権限を付与しているので、rtry-config は管理者権限なしで保存できる
- ログは `%TEMP%\rtry_debug.log` に置く（AppContainer からも書き込み可）
- **`%APPDATA%` は使うな**: MSIX パッケージ (Claude Desktop App 等) では `%LOCALAPPDATA%\Packages\<PFN>\LocalCache\Roaming\` に仮想化されてプロセスごとに別実体になる。`SHGetKnownFolderPath` の `KF_FLAG_NO_PACKAGE_REDIRECTION` でも回避できない（パス取得 API のリダイレクトしか防げず、ファイルアクセスは別機構で再仮想化される）
- ProgramData は Microsoft 仕様で MSIX/UWP のファイル仮想化対象外なので、AppContainer 含む全プロセスから同一実体として見える
- 修正案を出す前にファイルの書き込み先が適切か（権限の制約・MSIX 仮想化）を検証する
