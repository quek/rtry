...
- `crates/rtry-config/` - 設定 GUI（IMEインジケーター表示、3ストロークプレフィックスキー、40キーレイアウト）
...
- `vk_to_char` は VK コードを自然な文字に変換（VK_OEM_MINUS → `-`、VK_OEM_1 → `;`）
- 40キーレイアウトは `config.json` の `key_layout_40` で設定可能（デフォルト: `QWERTY_KEYS`）
- US キーボードでは position 29 を `-` に変更する必要がある
...
### 調査・デバッグ手順
- **安易な解決策を採用するな**。類似プロダクト（CorvusSKK, tsf-tutcode 等）の実装やベストプラクティスを調べてから設計・実装する
...
- `key_handler.rs` - キーイベント処理、IME オン/オフトグル、vk_to_char/vk_to_any_char 変換、交ぜ書きキー処理、VKBackBasedDeleter
...
- `language_bar.rs` - 言語バーボタン（2ボタンパターン: 独自GUID + GUID_LBI_INPUTMODE）、設定画面起動
- `function_provider.rs` - ITfFunctionProvider / ITfFnConfigure（Windows設定画面からの設定起動）
...
- 言語バーボタンと `is_open` を共有し、ON/OFF でアイコンを切り替え（「漢」⇔「A」）
...
- 1個の深さ2ネストセクション（40個の深さ3サブブロック）= ext table（3打鍵、プレフィックスキー接頭辞、デフォルトSpace）
- `@m` = 交ぜ書き変換, `@b` = 部首合成, `@q` = ヒストリ入力, `@!` = キャンセル

## 実装済み機能
- 2打鍵/3打鍵（プレフィックスキー接頭辞、デフォルトSpace）による直接漢字入力
...
- 設定 GUI（rtry-config.exe、言語バー右クリックまたは直接起動）
- 3ストロークプレフィックスキーの設定変更（デフォルト: Space、`config.json` の `ext_prefix_key` で指定）
- 40キーレイアウトの設定変更（`config.json` の `key_layout_40` で指定、設定GUIで編集可能）
- 言語バーボタン（GUID_LBI_INPUTMODE でタスクバーIMEインジケーターに統合、ON/OFF でアイコン切り替え）
...
## AppContainer 対応（スタートメニュー検索、Claude 等）
- SearchHost.exe 等の AppContainer プロセスは `%APPDATA%` にデフォルトではアクセスできない
- DLL・データファイルは `C:\Program Files\rtry\` に配置（AppContainer からデフォルトで読み取り可能）
- 設定ファイル検索順: DLL と同じディレクトリ → `%APPDATA%\rtry\`（フォールバック）
- **ACL による AppContainer 対応**（CorvusSKK 方式）: rtry-config が設定保存時に `%APPDATA%\rtry\` ディレクトリと `config.json` に `ALL_APP_PACKAGES`（`AC`）読み取り権限を SDDL で設定。API: `ConvertStringSecurityDescriptorToSecurityDescriptorW` → `GetSecurityDescriptorDacl` → `SetNamedSecurityInfoW`
- `install.bat` も `%APPDATA%\rtry\config.json` を DLL ディレクトリにコピー（フォールバック）
